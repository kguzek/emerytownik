...
from counterfactuals.models.emerytura_model import EmeryturaModel
...
def search_counterfactuals(
    ...
    X = dataset.destandardize_train_batch(torch.from_numpy(Xs_cfs))
    pd.DataFrame(X[0]).to_csv(counterfactuals_path, index=False)
...
@hydra.main(config_path="../../config", config_name="ppcef_config", version_base="1.2")
def main(cfg: DictConfig):
    ...
    dataset = DataModule.load_from_pickle(cfg.dataset.preprocessing_module_path)
    dataset.read_new_test(cfg.dataset.test_path)
    
    print("Dataset train shape >>>", dataset.X_train.shape)
    print("Dataset test shape >>>", dataset.X_test.shape)
    save_folder = cfg.counterfactuals_params.get("save_folder", save_folder)
    gen_model_path = cfg.gen_model.pretrained_path
    ...
    gen_model = EmeryturaModel.load(cfg.gen_model.pretrained_path)
    ...
    logger.info(f"Metrics for fold explanations")
